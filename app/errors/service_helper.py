        ...
        def wrapper(*args, **kwargs):
            ...
            while retry_count < max_retries:
                try:
                    ...
                    if retry_count < max_retries:
                        logger.warning(f'Retry {retry_count}/{max_retries} for {func.__name__} due to: {str(e)}')
                        time.sleep(delay)
                        delay *= 2
                    else:
            ...
            if last_exception:
                if isinstance(last_exception, ServiceError):
                    ...
                    raise ServiceError(f'Service operation failed after {max_retries} retries: {str(last_exception)}', operation=func.__name__) from last_exception
        return wrapper
    return decorator
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                logger.info(f'Service call completed: {service_name}.{operation} in {elapsed_time:.2f}s')
                return result
            except Exception as e:
                elapsed_time = time.time() - start_time
                log_exception(e, logger=logger, extra={'service': service_name, 'operation': operation, 'elapsed_time': elapsed_time})
                raise
        return wrapper
    return decorator
...
class ServiceBase:
    ...
    def _handle_service_error(self, operation, exception, **context):
        ...
        """
        log_exception(exception, logger=self.logger, extra={'operation': operation, **context})
        if isinstance(exception, ServiceError):
            return exception
        return ServiceError(f'{self.service_name} error in {operation}: {str(exception)}', service=self.service_name, operation=operation, **context)