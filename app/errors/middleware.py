...
class ErrorHandlingMiddleware:
    ...
    def before_request(self):
        ...
        g.context = {'request_id': g.request_id, 'user_agent': request.headers.get('User-Agent', 'Unknown'), 'remote_addr': request.remote_addr}

    def after_request(self, response):
        """Process the response after it's been generated by the view."""
        duration = time.time() - g.get('request_start_time', time.time())
        logger.info(f'Request {g.get('request_id', 'unknown')} completed: {request.method} {request.path} {response.status_code} in {duration:.2f}s')
    ...
    def teardown_request(self, exception=None):
        ...
        if exception:
            if isinstance(exception, HTTPException):
                return
            logger.error(f'Unhandled exception in request {g.get('request_id', 'unknown')}: {str(exception)}')
