...
def create_user():
    ...
    if form.validate_on_submit():
        ...
        # Create the user - admin-created users are automatically approved
        user = User(
        ...
        try:
            ...
            logger.info(f'Admin {current_user.username} created user {user.username} with temporary password')
            flash(f'User created successfully. Temporary password: {temp_password}', 'success')
        except Exception as e:
            logger.error(f'Failed to send welcome email to {user.email}: {str(e)}')
            flash(f'User created but could not send welcome email. Temporary password: {temp_password}', 'warning')
...
def toggle_admin(user_id):
    ...
    logger.info(f'Admin {current_user.username} {status} admin privileges for {user.username}')
...
def reset_password(user_id):
    ...
    flash(f'Password for {user.username} has been reset. New temporary password: {temp_password}', 'success')
    return redirect(url_for('admin.user_list'))