...
class UserAccessMiddleware:
    ...
    def before_request(self):
        ...
        if not current_user.is_approved:
            logger.info(f'User {current_user.username} blocked from accessing {request.path} - waiting for approval')
            return redirect(url_for('auth.pending_approval'))
            
        # Redirect to change password page if user has a temporary password
        if current_user.is_temporary_password:
            logger.info(f'User {current_user.username} redirected to change password page')
