...
def delete_file(file_id):
    ...
    if (
        request.referrer
        and url_for("files.file_detail", file_id=file_id) in request.referrer
    ):
        return redirect(url_for("files.file_list"))
    return redirect(url_for("files.file_list"))
...
def api_file_detail(file_id):
    ...
    if file.user_id != current_user.id:
        return (
            jsonify({"error": "You do not have permission to view this file."}),
            403,
        )
...
def api_models():
    ...
    try:
        ...
        if not subscription_key or not region:
            ...
            return (
                jsonify(
                    {"error": "Missing Azure Speech API configuration", "models": []}
                ),
                500,
            )
        ...
        for model in base_models:
            ...
            if not locale or not created_str:
                continue
            try:
                created_dt = datetime.fromisoformat(created_str.replace("Z", "+00:00"))
            except ValueError:
                logger.warning(
                    f"Could not parse createdDateTime '{created_str}' for base model"
                )
                continue
            model_description = (
                model.get("description") or model.get("name") or "Unknown Model"
            )
        ...
        return (
            jsonify(
                {
                    "error": f"An unexpected error occurred while retrieving models: {str(e)}",
                    "models": [],
                }
            ),
            500,
    ...
    if request.method == "POST":
        ...
        try:
            ...
            try:
                ...
                log_exception(e, logger)
                flash(
                    f"Warning: Error starting transcription process: {str(e)}",
                    "warning",
                )
            flash(
                "File uploaded successfully. Processing has started automatically.",
                "success",
            )
            return redirect(url_for("files.file_list"))
...
def start_upload():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if isinstance(
                e, (ValidationError, UploadError, StorageError, DatabaseError)
            ):
