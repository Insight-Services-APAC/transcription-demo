    ...
    try:
        ...
        if not progress_info:
            try:
                ...
                if legacy_progress:
                    return jsonify({'status': 'uploading' if legacy_progress['progress'] < 100 else 'completed', 'progress': legacy_progress['progress'], 'stage': 'azure_upload', 'uploaded_bytes': legacy_progress.get('uploaded_bytes', 0), 'total_bytes': legacy_progress.get('file_size', 0), 'message': 'Progress recovered from blob service'})
            except Exception as e:
                log_exception(e, logger)
        ...
        if progress_info.get('status') == 'completed':
            ...
            return jsonify({'status': 'completed', 'progress': 100, 'stage': 'complete', 'redirect_url': url_for('files.file_detail', file_id=file_id) if file_id else url_for('files.file_list')})
        if progress_info.get('azure_status') == 'in_progress':
            ...
            if time.time() - last_update > 10:
                ...
                return jsonify({'status': 'uploading', 'progress': current_progress, 'stage': 'azure_upload', 'uploaded_bytes': progress_info.get('uploaded_bytes', 0), 'total_bytes': progress_info.get('file_size', 0), 'message': 'Progress information may be delayed'})
            return jsonify({'status': 'uploading', 'progress': current_progress, 'stage': 'azure_upload', 'uploaded_bytes': progress_info.get('uploaded_bytes', 0), 'total_bytes': progress_info.get('file_size', 0)})
        ...
        return jsonify({'status': 'uploading', 'progress': progress_info.get('progress', 0), 'stage': progress_info.get('stage', 'unknown'), 'message': 'Upload in progress'})
...
def task_status(task_id):
    ...
    try:
        ...
        if task_result.state == 'PENDING':
            ...
            response = {'state': task_result.state, 'status': 'Error', 'error': error_info, 'traceback': task_result.traceback}
        elif task_result.state == 'SUCCESS':
            response = {'state': task_result.state, 'status': 'Completed', 'result': task_result.result}
        else:
            response = {'state': task_result.state, 'status': 'In Progress', 'info': task_result.info}
