...
class BatchTranscriptionService:
    ...
    def submit_transcription(self, audio_url, enable_diarization=True, model_id=None, locale='en-AU'):
        """
    ...
    def list_models(self, model_type="base"):
        ...
        while True:
            ...
            try:
                ...
                for model in batch_values:
                    is_deprecated = False
                    deprecation_dates = model.get('properties', {}).get('deprecationDates', {})
                    
                    # Check transcription deprecation date
                    transcription_date_str = deprecation_dates.get('transcriptionDateTime')
                    if transcription_date_str:
                        try:
                            transcription_date = datetime.datetime.strptime(transcription_date_str, "%Y-%m-%dT%H:%M:%SZ")
                            if transcription_date < current_date:
                                is_deprecated = True
                                self.logger.debug(f"Filtering out deprecated model: {model.get('displayName', 'Unknown')} (deprecated on {transcription_date_str})")
                        except ValueError:
                            # If date parsing fails, assume it's valid
                            self.logger.warning(f"Could not parse deprecation date: {transcription_date_str}")
        ...
        self.logger.info(f"Retrieved {len(models_list.get('values', []))} non-deprecated models out of {total_models} total models.")
        return models_list