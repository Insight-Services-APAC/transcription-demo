...
class BlobStorageService(ServiceBase):
    ...
    def __init__(self, connection_string, container_name):
        ...
        if not connection_string:
            raise ValidationError('Azure Storage connection string is required', field='connection_string')
        if not container_name:
            raise ValidationError('Azure Storage container name is required', field='container_name')
        try:
            ...
            raise StorageError(f'Failed to initialize Azure Blob Storage client: {str(e)}', container=container_name)
    ...
    def upload_file(self, file_path, blob_path, upload_id=None, progress_tracker=None):
        ...
        try:
            ...
            raise StorageError(f'Failed to create blob client: {str(e)}', container=self.container_name, blob_path=blob_path)
        ...
        def progress_callback(current):
            ...
            if upload_id:
                if progress_tracker:
                    progress_data = {'status': 'uploading', 'progress': min(99, int(uploaded_bytes / file_size * 100)), 'file_size': file_size, 'uploaded_bytes': uploaded_bytes, 'stage': 'azure_upload', 'azure_status': 'in_progress'}
                    progress_tracker.update_progress(upload_id, progress_data)
                else:
                    with self.upload_lock:
                        if upload_id not in self.upload_progress:
                            self.upload_progress[upload_id] = {'status': 'uploading', 'progress': 0, 'file_size': file_size, 'uploaded_bytes': 0, 'last_update': time.time()}
        ...
        try:
            with open(file_path, 'rb') as data:
                blob_client.upload_blob(data, overwrite=True, content_settings=ContentSettings(content_type=content_type), max_concurrency=1, raw_response_hook=progress_callback)
            logger.info(f'Upload completed. callback_count={callback_count}, total bytes={uploaded_bytes}')
            if upload_id:
                if progress_tracker:
                    progress_data = {'status': 'uploading', 'progress': 100, 'file_size': file_size, 'uploaded_bytes': file_size, 'stage': 'finalizing', 'azure_status': 'completed'}
                    progress_tracker.update_progress(upload_id, progress_data)
                else:
                    with self.upload_lock:
            ...
            sas_token = generate_blob_sas(account_name=account_name, container_name=self.container_name, blob_name=blob_path, account_key=account_key, permission=BlobSasPermissions(read=True), expiry=expiry_time)
            ...
            if upload_id:
                if progress_tracker:
                    progress_data = {'status': 'error', 'azure_status': 'error', 'error': str(e)}
                    progress_tracker.update_progress(upload_id, progress_data)
                else:
                    with self.upload_lock:
                        if upload_id in self.upload_progress:
            ...
            raise StorageError(f'Error uploading file to Azure storage: {str(e)}', file_path=file_path, blob_path=blob_path, container=self.container_name)
    ...
    def download_file(self, blob_path, local_path):
        ...
        try:
            ...
            raise StorageError(f'Error downloading blob: {str(e)}', blob_path=blob_path, local_path=local_path, container=self.container_name)
    ...
    def delete_blob(self, blob_path):
        ...
        try:
            ...
            raise StorageError(f'Error deleting blob: {str(e)}', blob_path=blob_path, container=self.container_name)
    ...
    def upload_bytes(self, data, blob_path, content_type=None):
        ...
        try:
            blob_client = self.blob_service_client.get_blob_client(container=self.container_name, blob=blob_path)
            blob_client.upload_blob(data, overwrite=True, content_settings=ContentSettings(content_type=content_type) if content_type else None)
            account_name = self.blob_service_client.account_name
            account_key = self.blob_service_client.credential.account_key
            expiry_time = datetime.utcnow() + timedelta(hours=24)
            sas_token = generate_blob_sas(account_name=account_name, container_name=self.container_name, blob_name=blob_path, account_key=account_key, permission=BlobSasPermissions(read=True), expiry=expiry_time)
            ...
            raise StorageError(f'Error uploading bytes to Azure storage: {str(e)}', blob_path=blob_path, content_type=content_type, container=self.container_name)
    ...
    def _get_content_type(self, file_path):
        ...
        content_types = {'.wav': 'audio/wav', '.mp3': 'audio/mpeg', '.json': 'application/json', '.txt': 'text/plain'}
