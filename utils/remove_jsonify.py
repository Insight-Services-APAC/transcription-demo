...
class JsonifyRemover(ast.NodeTransformer):
    ...
    def visit_Assign(self, node):
        if isinstance(node.value, ast.Call) and isinstance(node.value.func, ast.Name) and (node.value.func.id == 'jsonify') and (len(node.value.args) == 1) and isinstance(node.value.args[0], (ast.Dict, ast.Call)):
